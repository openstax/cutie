...
exports.formatXmlDom = formatXmlDom;
const withQtiInteractions_1 = require("../plugins/withQtiInteractions");
const xmlToSlate_1 = require("./xmlToSlate");
/**
...
function buildBlockElementSet() {
    ...
    for (const config of withQtiInteractions_1.elementConfigs) {
        if (!config.isInline) {
    ...
    for (const tag of xmlToSlate_1.BLOCK_ELEMENTS_FOR_WHITESPACE) {
        blockSet.add(tag);
    }
 ...
 */
function formatXmlDom(element, depth = 0, indent = '  ') {
    var _a;
