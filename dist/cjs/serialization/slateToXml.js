...
exports.serializeSlateToXml = serializeSlateToXml;
exports.serializeSlateToQti = serializeSlateToQti;
...
const formatXmlDom_1 = require("./formatXmlDom");
 ...
 */
function isDocumentMetadata(node) {
...
function isEmptyParagraph(node) {
    ...
    const element = node;
    return element.children.every((child) => 'text' in child && child.text === '');
}
/**
 ...
 */
function stripEmptySpacerParagraphs(nodes) {
 ...
 */
function serializeSlateToXml(nodes) {
    const doc = (0, xmlUtils_1.createXmlDocument)();
...
const interactionSerializers = {
    ...choice_1.choiceSerializers,
    ...textEntry_1.textEntrySerializers,
    ...inlineChoice_1.inlineChoiceSerializers,
    ...extendedText_1.extendedTextSerializers,
    ...gapMatch_1.gapMatchSerializers,
    ...match_1.matchSerializers,
    ...prompt_1.promptSerializers,
    ...simpleChoice_1.simpleChoiceSerializers,
    ...feedbackInline_1.feedbackInlineSerializers,
    ...feedbackBlock_1.feedbackBlockSerializers,
    ...modalFeedback_1.modalFeedbackSerializers,
    ...contentBody_1.contentBodySerializers,
};
/**
 * Convert a Slate node to an XML element or text node
 */
function convertSlateNodeToXml(node, context) {
    // Text node
    if ('text' in node) {
        return convertTextNodeToXml(node, context);
    }
    // Element node
    const element = node;
    // Helper to convert children
    const convertChildren = (children, parent) => {
 ...
 */
function convertTextNodeToXml(node, context) {
    let textNode = context.doc.createTextNode(node.text);
    // Wrap in formatting elements if needed (use QTI namespace for all elements)
    if (node.bold) {
        const strong = (0, xmlUtils_1.createXmlElement)(context.doc, 'strong');
        strong.appendChild(textNode);
        textNode = strong;
    }
    if (node.italic) {
        const em = (0, xmlUtils_1.createXmlElement)(context.doc, 'em');
        em.appendChild(textNode);
        textNode = em;
    }
    if (node.underline) {
        const u = (0, xmlUtils_1.createXmlElement)(context.doc, 'u');
        u.appendChild(textNode);
        textNode = u;
    }
    if (node.code) {
        const code = (0, xmlUtils_1.createXmlElement)(context.doc, 'code');
        code.appendChild(textNode);
        textNode = code;
    }
    if (node.strikethrough) {
        const s = (0, xmlUtils_1.createXmlElement)(context.doc, 's');
 ...
 */
function convertUnknownQtiElement(element, context) {
    ...
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, element.originalTagName);
...
function convertParagraph(element, context) {
    var _a;
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'p');
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    // Add alignment to style attribute
    const style = buildStyleWithAlignment((_a = element.attributes) === null || _a === void 0 ? void 0 : _a['style'], element.align);
    if (style) {
...
function convertDiv(element, context) {
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'div');
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    // Convert children
 ...
 */
function convertSpan(element, context) {
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'span');
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    // Convert children
...
function convertHeading(element, context) {
    var _a;
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, `h${element.level}`);
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    // Add alignment to style attribute
...
function convertImage(element, context) {
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'img');
    setAttributes(xmlElement, element.attributes);
    return xmlElement;
}
/**
 ...
 */
function convertLineBreak(element, context) {
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'br');
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    return xmlElement;
 ...
 */
function convertList(element, context) {
    const tagName = element.ordered ? 'ol' : 'ul';
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, tagName);
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    // Convert children
...
function convertListItem(element, context) {
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'li');
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    // Convert children
 ...
 */
function convertStrong(element, context) {
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'strong');
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    // Convert children
 ...
 * Convert em element
 */
function convertEm(element, context) {
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'em');
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    // Convert children
...
function convertBlockquote(element, context) {
    var _a;
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'blockquote');
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    // Add alignment to style attribute
 ...
 */
function convertHorizontalRule(element, context) {
    const xmlElement = (0, xmlUtils_1.createXmlElement)(context.doc, 'hr');
    if (element.attributes) {
        setAttributes(xmlElement, element.attributes);
    }
    return xmlElement;
 ...
 */
function buildStyleWithAlignment(existingStyle, align) {
 ...
 * Check for duplicate response identifiers
 */
function checkDuplicateIdentifiers(context) {
    const seen = new Map();
 ...
 */
function updateResponseDeclarations(assessmentItem, declarations, doc) {
    ...
    for (const [, node] of declarations) {
        const element = (0, xmlNode_1.xmlNodeToDom)(node, doc);
 ...
 * @param doc - The XML document
 */
function updateOutcomeDeclarations(assessmentItem, declarations, doc) {
    const itemBody = assessmentItem.querySelector('qti-item-body');
    ...
    for (const [identifier] of declarations) {
        const existing = assessmentItem.querySelector(`qti-outcome-declaration[identifier="${identifier}"]`);
        if (existing) {
            existing.remove();
        }
    }
 ...
 * @param doc - The XML document
 */
function updateFeedbackOutcomeDeclaration(assessmentItem, feedbackIdentifiersUsed, doc) {
    const itemBody = assessmentItem.querySelector('qti-item-body');
    const existingFeedback = assessmentItem.querySelector('qti-outcome-declaration[identifier="FEEDBACK"]');
    if (feedbackIdentifiersUsed.size > 0) {
        // Need FEEDBACK outcome declaration
        if (!existingFeedback) {
            // Create new FEEDBACK outcome declaration
            const feedbackDecl = doc.createElementNS('http://www.imsglobal.org/xsd/imsqtiasi_v3p0', 'qti-outcome-declaration');
 ...
 */
function updateResponseProcessing(assessmentItem, config, responseIdentifiers, responseDeclarations, outcomeDeclarations, feedbackIdentifiersUsed, responseConfigs, doc) {
    ...
    if (config) {
        const newRP = (0, responseProcessingGenerator_1.generateResponseProcessingXml)(config, responseIdentifiers, responseDeclarations, outcomeDeclarations, doc, feedbackIdentifiersUsed, responseConfigs);
        if (newRP) {
...
/**
 * Update modal feedback elements in the assessment item.
 * Modal feedback lives outside qti-item-body, before qti-response-processing.
 *
 * @param assessmentItem - The qti-assessment-item element
 * @param modalFeedbackElements - Array of modal feedback elements to insert
 * @param doc - The XML document
 */
function updateModalFeedback(assessmentItem, modalFeedbackElements, doc) {
    // Remove existing modal feedback elements
    const existingModalFeedback = assessmentItem.querySelectorAll('qti-modal-feedback');
    for (const el of existingModalFeedback) {
        el.remove();
    }
    // Insert new modal feedback elements before response-processing (or at end)
    const responseProcessing = assessmentItem.querySelector('qti-response-processing');
    for (const element of modalFeedbackElements) {
        const imported = doc.importNode(element, true);
        if (responseProcessing) {
            assessmentItem.insertBefore(imported, responseProcessing);
        }
        else {
            assessmentItem.appendChild(imported);
        }
    }
}
/**
 ...
 */
function serializeSlateToQti(nodes, originalQtiXml) {
    var _a;
    ...
    updateResponseProcessing(assessmentItem, itemBodyResult.responseProcessingConfig, itemBodyResult.responseIdentifiers, itemBodyResult.responseDeclarations, itemBodyResult.outcomeDeclarations, itemBodyResult.feedbackIdentifiersUsed, itemBodyResult.responseConfigs, doc);
    ...
    if (((_a = itemBodyResult.responseProcessingConfig) === null || _a === void 0 ? void 0 : _a.mode) !== 'custom') {
        updateFeedbackOutcomeDeclaration(assessmentItem, itemBodyResult.feedbackIdentifiersUsed, doc);
    }
    ...
    // Update modal feedback elements (inserted outside qti-item-body, before response-processing)
    updateModalFeedback(assessmentItem, itemBodyResult.modalFeedbackElements, doc);
    ...
    (0, formatXmlDom_1.formatXmlDom)(doc.documentElement);
