 ...
 * @returns The newly focused element, or null if the group is empty
 */
function focusPrev(elements, currentId) {
    const ids = Array.from(elements.keys());
    if (ids.length === 0)
        return null;
    const currentIndex = ids.indexOf(currentId);
 ...
 */
function updateRovingTabindex(elements, focusedElement) {
 ...
 */
function initializeRovingTabindex(elements) {
    let isFirst = true;
    for (const element of elements.values()) {
