...
exports.generateResponseProcessingXml = generateResponseProcessingXml;
exports.createMatchElement = createMatchElement;
exports.createMapResponseEqualElement = createMapResponseEqualElement;
exports.createMapResponseGtZeroElement = createMapResponseGtZeroElement;
exports.createSetFeedbackElement = createSetFeedbackElement;
const xmlNode_1 = require("../serialization/xmlNode");
const feedbackIdentifiers_1 = require("./feedbackIdentifiers");
const mappingDeclaration_1 = require("./mappingDeclaration");
/**
 * QTI namespace URI
 */
 ...
 */
function generateResponseProcessingXml(config, responseIdentifiers, responseDeclarations, outcomeDeclarations, doc, feedbackIdentifiersUsed, responseConfigs) {
    ...
    switch (config.mode) {
        ...
        case 'allCorrect':
            return generateAllCorrectXml(responseIdentifiers, responseDeclarations, doc, feedbackIdentifiersUsed, responseConfigs);
        case 'sumScores':
            return generateSumScoresXml(responseIdentifiers, responseDeclarations, outcomeDeclarations, doc, feedbackIdentifiersUsed, responseConfigs);
        default:
            return null;
    }
}
 ...
 */
function generateCustomXml(config, doc) {
    if (!config.customXml) {
        return null;
    }
    return (0, xmlNode_1.xmlNodeToDom)(config.customXml, doc);
}
/**
 ...
 */
function generateAllCorrectXml(responseIdentifiers, responseDeclarations, doc, feedbackIdentifiersUsed, responseConfigs) {
    ...
    // Check if any response has a mapping
    const anyHasMapping = responseIdentifiers.some(id => {
        const decl = responseDeclarations.get(id);
        return decl && (0, mappingDeclaration_1.hasMapping)(decl);
    });
    // Template shortcut: only when no mapping, no feedback, and single RESPONSE
    if (responseIdentifiers.length === 1 && responseIdentifiers[0] === 'RESPONSE' && !hasFeedback && !anyHasMapping) {
        responseProcessing.setAttribute('template', TEMPLATES.matchCorrect);
        return responseProcessing;
    }
    // Generate inline pattern
    if (responseIdentifiers.length > 0) {
        ...
        if (responseIdentifiers.length === 1) {
            // Single interaction: use correctness check directly
            responseIf.appendChild(createCorrectnessCheckForId(responseIdentifiers[0], responseDeclarations, responseConfigs, doc));
        }
        else {
            // Multiple interactions: use qti-and
            const and = doc.createElementNS(QTI_NAMESPACE, 'qti-and');
            for (const id of responseIdentifiers) {
                and.appendChild(createCorrectnessCheckForId(id, responseDeclarations, responseConfigs, doc));
            }
    ...
    if (hasFeedback) {
        const feedbackConditions = generateFeedbackProcessingXml(responseIdentifiers, responseDeclarations, feedbackIdentifiersUsed, doc, undefined, responseConfigs);
 ...
 */
function createOutcomeDeclaration(identifier) {
    return {
 ...
 */
function generateSumScoresXml(responseIdentifiers, responseDeclarations, outcomeDeclarations, doc, feedbackIdentifiersUsed, responseConfigs) {
    const responseProcessing = doc.createElementNS(QTI_NAMESPACE, 'qti-response-processing');
    const hasFeedback = feedbackIdentifiersUsed && feedbackIdentifiersUsed.size > 0;
    ...
    if (responseIdentifiers.length === 1 && responseIdentifiers[0] === 'RESPONSE' && !hasFeedback) {
        ...
        const decl = responseDeclarations.get(id);
        const hasMappingDefined = decl && (0, mappingDeclaration_1.hasMapping)(decl);
    ...
    for (const id of responseIdentifiers) {
        const decl = responseDeclarations.get(id);
        if (decl && (0, mappingDeclaration_1.hasMapping)(decl)) {
            withMapping.push(id);
        }
        else {
    ...
    // Generate feedback processing rules if feedback elements are used
    if (hasFeedback) {
        const feedbackConditions = generateFeedbackProcessingXml(responseIdentifiers, responseDeclarations, feedbackIdentifiersUsed, doc, new Set(withMapping), responseConfigs);
        for (const fc of feedbackConditions) {
            responseProcessing.appendChild(fc);
        }
    }
...
/**
 * Create the correctness check element for a given response identifier.
 * Delegates to config.generateCorrectnessCheck if available,
 * otherwise uses mapping-based or match-based default.
 */
function createCorrectnessCheckForId(id, responseDeclarations, responseConfigs, doc) {
    const decl = responseDeclarations.get(id);
    const config = responseConfigs === null || responseConfigs === void 0 ? void 0 : responseConfigs.get(id);
    // Use config method if available
    if ((config === null || config === void 0 ? void 0 : config.generateCorrectnessCheck) && decl) {
        return config.generateCorrectnessCheck(id, decl, doc);
    }
    // Default: if mapping exists, use qti-equal(qti-map-response, maxValue)
    if (decl && (0, mappingDeclaration_1.hasMapping)(decl)) {
        const maxValue = (0, mappingDeclaration_1.getMaxMappedValue)(decl);
        if (maxValue !== null) {
            return createMapResponseEqualElement(id, maxValue, doc);
        }
    }
    // Default: qti-match(variable, correct)
    return createMatchElement(id, doc);
}
/**
 * Create a qti-match element that compares a variable to its correct value
 */
function createMatchElement(identifier, doc) {
 ...
 * </qti-equal>
 */
function createMapResponseEqualElement(identifier, value, doc) {
    const equal = doc.createElementNS(QTI_NAMESPACE, 'qti-equal');
    const mapResponse = doc.createElementNS(QTI_NAMESPACE, 'qti-map-response');
    mapResponse.setAttribute('identifier', identifier);
    equal.appendChild(mapResponse);
    const baseValue = doc.createElementNS(QTI_NAMESPACE, 'qti-base-value');
    baseValue.setAttribute('base-type', 'float');
    baseValue.textContent = String(value);
    equal.appendChild(baseValue);
    return equal;
}
/**
 * Create a qti-set-outcome-value element for SCORE
 */
function createSetScoreElement(value, doc) {
    const setScore = doc.createElementNS(QTI_NAMESPACE, 'qti-set-outcome-value');
    setScore.setAttribute('identifier', 'SCORE');
    const baseValue = doc.createElementNS(QTI_NAMESPACE, 'qti-base-value');
    baseValue.setAttribute('base-type', 'float');
 ...
 * </qti-response-condition>
 */
function createScoreConditionForUnmapped(identifier, doc) {
    const condition = doc.createElementNS(QTI_NAMESPACE, 'qti-response-condition');
    // Response if
    const responseIf = doc.createElementNS(QTI_NAMESPACE, 'qti-response-if');
 ...
 */
function generateFeedbackProcessingXml(responseIdentifiers, responseDeclarations, feedbackIdentifiersUsed, doc, mappedResponseIds, responseConfigs) {
    var _a, _b;
    const conditions = [];
    ...
    for (const responseId of responseIdentifiers) {
        ...
        if (!feedbackIds || feedbackIds.size === 0)
            continue;
        const responseDecl = responseDeclarations.get(responseId);
        const config = responseConfigs === null || responseConfigs === void 0 ? void 0 : responseConfigs.get(responseId);
        // Delegate to config if it provides generateFeedbackConditions
        if ((config === null || config === void 0 ? void 0 : config.generateFeedbackConditions) && responseDecl) {
            const configConditions = config.generateFeedbackConditions(responseId, responseDecl, feedbackIds, doc);
            conditions.push(...configConditions);
            continue;
        }
        // Default feedback generation
        const cardinality = ((_a = responseDecl === null || responseDecl === void 0 ? void 0 : responseDecl.attributes) === null || _a === void 0 ? void 0 : _a.cardinality) || 'single';
        ...
        if (hasCorrect || hasIncorrect) {
            ...
            const useMapResponse = (_b = mappedResponseIds === null || mappedResponseIds === void 0 ? void 0 : mappedResponseIds.has(responseId)) !== null && _b !== void 0 ? _b : false;
            const condition = createFeedbackCorrectIncorrectCondition(responseId, hasCorrect ? correctId : null, hasIncorrect ? incorrectId : null, doc, useMapResponse);
            conditions.push(condition);
        }
        // Check for per-choice feedback
        for (const feedbackId of feedbackIds) {
            const parsed = (0, feedbackIdentifiers_1.parseFeedbackIdentifier)(feedbackId);
            if ((parsed === null || parsed === void 0 ? void 0 : parsed.type) === 'choice' && parsed.choiceId) {
                const condition = createFeedbackChoiceCondition(responseId, parsed.choiceId, feedbackId, cardinality, doc);
                conditions.push(condition);
            }
        }
    }
 ...
 * </qti-response-condition>
 */
function createFeedbackCorrectIncorrectCondition(responseId, correctFeedbackId, incorrectFeedbackId, doc, useMapResponse = false) {
    const condition = doc.createElementNS(QTI_NAMESPACE, 'qti-response-condition');
 ...
 */
function createMapResponseGtZeroElement(identifier, doc) {
 ...
 * </qti-response-condition>
 */
function createFeedbackChoiceCondition(responseId, choiceId, feedbackId, cardinality, doc) {
    const condition = doc.createElementNS(QTI_NAMESPACE, 'qti-response-condition');
 ...
 * </qti-set-outcome-value>
 */
function createSetFeedbackElement(feedbackId, doc) {
