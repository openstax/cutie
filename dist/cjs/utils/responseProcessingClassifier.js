...
exports.classifyResponseProcessing = classifyResponseProcessing;
 ...
 */
function classifyResponseProcessing(doc) {
    ...
    if (template) {
        ...
        return {
            mode: 'custom',
            customXml: (0, xmlNode_1.domToXmlNode)(responseProcessing),
        };
    }
 ...
 */
function classifyTemplate(templateUrl) {
 ...
 */
function classifyInlinePattern(responseProcessing) {
 ...
 */
function isIntermediateScoreCondition(condition) {
 ...
 */
function isSumScoresPattern(responseProcessing) {
 ...
 * optionally followed by feedback-only conditions
 */
function isAllCorrectPattern(responseProcessing) {
 ...
 */
function isAllCorrectScoringCondition(condition) {
    ...
    if (and) {
        ...
        for (const child of and.children) {
            ...
            if (tag === 'qti-match' && isMatchVariableToCorrect(child))
                continue;
            if (tag === 'qti-equal' && isMapResponseEqualToValue(child))
                continue;
            return false;
        }
 ...
 */
function isMatchVariableToCorrect(match) {
 ...
 */
function isMapResponseEqualToValue(equal) {
 ...
 */
function hasMapping(responseDeclaration) {
    return responseDeclaration.children.some((child) => typeof child !== 'string' && child.tagName === 'qti-mapping');
}
/**
 ...
 */
function areFeedbackPatternsStandard(responseProcessing) {
    // Find all qti-set-outcome-value elements that set FEEDBACK
    const feedbackSetters = responseProcessing.querySelectorAll('qti-set-outcome-value[identifier="FEEDBACK"]');
 ...
 */
function isFeedbackOnlyCondition(condition) {
 ...
 * </qti-set-outcome-value>
 */
function isStandardFeedbackSetter(setter) {
    var _a;
    ...
    const identifierValue = ((_a = baseValue.textContent) === null || _a === void 0 ? void 0 : _a.trim()) || '';
    if (!(0, feedbackIdentifiers_1.isStandardFeedbackIdentifier)(identifierValue)) {
        return false;
    }
    return true;
}
