...
const index_1 = require("./expressionEvaluator/index");
 ...
 */
function initializeState(itemDoc) {
    const variables = {};
    ...
    const score = (0, scoreUtils_1.extractStandardOutcomes)(variables, itemDoc);
 ...
 */
function initializeOutcomeVariables(itemDoc, variables) {
        ...
        if (defaultValueElement) {
            ...
            if (cardinality === 'single') {
                ...
                if (valueElement && baseType) {
                    variables[identifier] = (0, typeParser_1.parseValue)(valueElement.textContent || '', baseType);
                }
            }
                ...
                for (let j = 0; j < valueElements.length; j++) {
                    if (baseType) {
                        values.push((0, typeParser_1.parseValue)(valueElements[j].textContent || '', baseType));
                    }
                }
                ...
                for (let j = 0; j < fieldElements.length; j++) {
                    ...
                    if (fieldIdentifier && valueElement) {
                        ...
                        if (fieldBaseType) {
                            record[fieldIdentifier] = (0, typeParser_1.parseValue)(valueElement.textContent || '', fieldBaseType);
                        }
                    }
                }
 ...
 */
function resetTemplateVariables(itemDoc, variables) {
 ...
 */
function executeTemplateProcessing(itemDoc, variables) {
    ...
    try {
        ...
        for (const child of (0, dom_1.getChildElements)(templateProcessing)) {
 ...
 */
function executeTemplateRule(rule, itemDoc, variables) {
 ...
 */
function executeSetTemplateValue(rule, itemDoc, variables) {
    const identifier = rule.getAttribute('identifier');
    if (!identifier)
        return;
    // Evaluate the expression (first child element)
    const child = (0, dom_1.getFirstChildElement)(rule);
    if (!child) {
 ...
 */
function executeTemplateCondition(rule, itemDoc, variables) {
    // Find template-if, template-else-if, and template-else elements
    for (const child of (0, dom_1.getChildElements)(rule)) {
 ...
 */
function evaluateTemplateIf(element, itemDoc, variables) {
 ...
 */
function executeTemplateBlock(element, itemDoc, variables) {
    for (const child of (0, dom_1.getChildElements)(element)) {
        executeTemplateRule(child, itemDoc, variables);
    }
}
 ...
 */
function executeTemplateConstraint(rule, itemDoc, variables) {
 ...
 */
function executeSetCorrectResponse(rule, itemDoc, variables) {
    const identifier = rule.getAttribute('identifier');
    if (!identifier)
        return;
 ...
 */
function executeSetDefaultValue(rule, itemDoc, variables) {
    const identifier = rule.getAttribute('identifier');
    if (!identifier)
        return;
...
function evaluateExpression(element, itemDoc, variables) {
    ...
    return (0, index_1.evaluateExpression)(element, itemDoc, variables, subEvaluate);
}
/**
 ...
 */
function initializeShuffleOrders(itemDoc) {
    const shuffleOrders = {};
 ...
 */
function processChoiceInteractions(itemDoc, shuffleOrders) {
    const interactions = itemDoc.getElementsByTagName('qti-choice-interaction');
    for (let i = 0; i < interactions.length; i++) {
        const interaction = interactions[i];
        if (interaction.getAttribute('shuffle') !== 'true')
            continue;
        ...
        if (items.length > 0) {
            shuffleOrders[responseId] = (0, shuffle_1.generateShuffleOrder)(items);
        }
    }
}
/**
 ...
 */
function processInlineChoiceInteractions(itemDoc, shuffleOrders) {
 ...
 */
function processMatchInteractions(itemDoc, shuffleOrders) {
    ...
    for (let i = 0; i < interactions.length; i++) {
        ...
        for (let setIndex = 0; setIndex < matchSets.length; setIndex++) {
            ...
            if (items.length > 0) {
                ...
                shuffleOrders[`${responseId}_${setIndex}`] = (0, shuffle_1.generateShuffleOrder)(items);
            }
        }
    }
}
 ...
 */
function processGapMatchInteractions(itemDoc, shuffleOrders) {
