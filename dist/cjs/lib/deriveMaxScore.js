...
exports.deriveMaxScore = deriveMaxScore;
/**
 ...
 */
function deriveMaxScore(itemDoc, variables) {
    // 1. Check for explicit MAXSCORE variable
 ...
 */
function tryDeriveFromNormalMaximum(itemDoc) {
 ...
 */
function tryDeriveSumPattern(itemDoc) {
 ...
 */
function isSumOfVariables(element) {
 ...
 */
function findOutcomeAssignments(itemDoc, identifier) {
    const assignments = itemDoc.getElementsByTagName('qti-set-outcome-value');
    const matches = [];
    for (let i = 0; i < assignments.length; i++) {
        const assignment = assignments[i];
 ...
 */
function extractBaseValueFloats(assignments) {
    const values = [];
 ...
 */
function tryDeriveMappingUpperBound(itemDoc) {
 ...
 */
function tryDeriveSumOfMapEntries(itemDoc) {
    const responseDeclarations = itemDoc.getElementsByTagName('qti-response-declaration');
    for (let i = 0; i < responseDeclarations.length; i++) {
        const declaration = responseDeclarations[i];
        const mappingElements = declaration.getElementsByTagName('qti-mapping');
 ...
 */
function tryDeriveFromTemplate(itemDoc) {
    var _a;
    ...
    const templateName = ((_a = template.split('/').pop()) === null || _a === void 0 ? void 0 : _a.replace('.xml', '')) || '';
    // match_correct template and similar templates always score 0 or 1
    if (templateName === 'match_correct' ||
