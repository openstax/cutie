...
import { elementNeedsSpacers, getElementCategories, getElementForbiddenDescendants, normalizeElement, } from './withQtiInteractions';
/**
 ...
 */
export function withXhtml(editor) {
    ...
    editor.normalizeNode = (entry) => {
        ...
        if (Editor.isEditor(node) || (Element.isElement(node) && 'type' in node)) {
            const children = Editor.isEditor(node) ? node.children : node.children;
            ...
            if (Element.isElement(node) && 'type' in node && skipTypes.includes(node.type)) {
                ...
                for (let i = 0; i < children.length; i++) {
                    ...
                    if (elementNeedsSpacers(child)) {
                        ...
                        if (needsSpacerBefore) {
                            Transforms.insertNodes(editor, { type: 'paragraph', children: [{ text: '' }] }, { at: path.concat(i) });
                            return; // Normalization will re-run
                        }
                        ...
                        if (needsSpacerAfter) {
                            Transforms.insertNodes(editor, { type: 'paragraph', children: [{ text: '' }] }, { at: path.concat(i + 1) });
                            return; // Normalization will re-run
                        }
                    }
                }
        ...
        if (Element.isElement(node) && 'type' in node) {
            ...
            if (type === 'qti-choice-interaction') {
                const hasChoice = node.children.some((child) => Element.isElement(child) &&
                    'type' in child &&
