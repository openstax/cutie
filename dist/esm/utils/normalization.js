 ...
 */
export function wrapInlineContentInParagraphs(editor, node, path) {
    // If empty, insert a paragraph child
    if (node.children.length === 0) {
        Transforms.insertNodes(editor, { type: 'paragraph', children: [{ text: '' }], attributes: {} }, { at: path.concat(0) });
        return true;
    }
    ...
    for (let i = 0; i < node.children.length; i++) {
        ...
        const isInlineContent = Text.isText(child) || (Element.isElement(child) && !Editor.isBlock(editor, child));
        if (isInlineContent) {
            if (runStart === null) {
                runStart = i;
            }
    ...
    if (runStart !== null && runEnd !== null) {
        ...
        Transforms.wrapNodes(editor, { type: 'paragraph', children: [], attributes: {} }, {
            at: path,
