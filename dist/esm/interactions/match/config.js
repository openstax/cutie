...
export const matchInteractionConfig = {
    ...
    normalize: (editor, node, path) => {
        ...
        const hasSourceSet = children.some((child) => Element.isElement(child) &&
            'type' in child &&
            child.type === 'match-source-set');
        const hasTargetSet = children.some((child) => Element.isElement(child) &&
            'type' in child &&
        ...
        if (!hasTargetSet) {
            ...
            const sourceSetIndex = children.findIndex((child) => Element.isElement(child) &&
                'type' in child &&
                child.type === 'match-source-set');
            Transforms.insertNodes(editor, {
...
export const matchSourceSetConfig = {
    type: 'match-source-set',
    xmlTagName: null,
    isVoid: false,
    isInline: false,
...
export const matchTargetSetConfig = {
    type: 'match-target-set',
    xmlTagName: null,
    isVoid: false,
    isInline: false,
...
export const simpleAssociableChoiceConfig = {
    type: 'qti-simple-associable-choice',
