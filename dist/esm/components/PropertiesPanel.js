 ...
 */
export function PropertiesPanel({ selectedElement, selectedPath, onUpdateAttributes, responseProcessingConfig, interactionCount = 0, hasFeedbackElements = false, onResponseProcessingModeChange, }) {
    ...
    if (!selectedElement || !selectedPath) {
        ...
        if (responseProcessingConfig && onResponseProcessingModeChange) {
            return (_jsx("div", { className: "properties-panel", children: _jsx(ResponseProcessingPanel, { config: responseProcessingConfig, interactionCount: interactionCount, hasFeedbackElements: hasFeedbackElements, onModeChange: onResponseProcessingModeChange }) }));
        }
        return (_jsx("div", { className: "properties-panel", children: _jsx("div", { className: "properties-panel-empty", children: "Select an interaction to edit its properties" }) }));
    }
    ...
    if (Panel) {
        ...
        return (_jsx("div", { className: "properties-panel", children: _jsx(Panel, { element: selectedElement, path: selectedPath, onUpdate: onUpdateAttributes, ...(isFeedbackElement && { responseProcessingConfig }) }) }));
    }
